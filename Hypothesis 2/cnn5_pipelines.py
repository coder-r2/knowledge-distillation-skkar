...
def run_pipeline1(full_train, test_set, device, alpha, beta, gamma, delta):
    ...
    for step in range(NUM_STEPS):
        ...
        if step == 0:
            model = CNN5(num_classes=num_total).to(device)
            ...
            student = CNN5(num_classes=num_total).to(device)
...
if __name__ == "__main__":
    ...
    print(f"CNN5 Pipeline 1 — 10-class test accuracy: {acc1*100:.2f}%")

    acc2 = run_pipeline2(full_train, test_set, device, alpha=ALPHA, beta=BETA, gamma=GAMMA, delta=DELTA)
    print(f"CNN5 Pipeline 2 — 10-class test accuracy: {acc2*100:.2f}%")
